...
export default function App() {
  ...
  const handleInputChange = event => {
    ...
    if (id === 'lat') {
      ...
      axios
        .get(`https://api.open-elevation.com/api/v1/lookup?locations=${center[1]},${center[0]}`)
        .then(response => {
          const heightWithRelef = response.data.results[0].elevation + height;
          setHeight(heightWithRelef);
        });
    } else if (id === 'corner') {
  ...
  return (
    ...
    >
      {markers.map((marker, index) => (
        <Marker key={index} latitude={marker.coordinates[1]} longitude={marker.coordinates[0]}>
          <div
            style={{
      ...
      >
        <TextField fullWidth label="Lat" id="lat" value={center[1]} onChange={handleInputChange} />
        <TextField fullWidth label="Lon" id="lon" value={center[0]} onChange={handleInputChange} />
        <TextField
          fullWidth
          label="Радиус (м)"
          id="radius"
